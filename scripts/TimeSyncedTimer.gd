...
##Leave as -1 unless you want the timer to initially spawn with some other time left
@export var time_left : float = -1
@export var one_shot : bool = false
##Timer is started during ready() call
@export var autostart : bool = false
var paused : bool = false
var running : bool = false
signal timeout()
##if time reverses back to before the timer was started. Both signals will be emitted if a non-one_shot timer is reversed
